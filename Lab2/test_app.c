...

{
	...
	for (i = 0; i < 4096; i++)
 	...
 	if (ioctl(fd, RD_LSMOD, &user_modules.module_names) < 0) {
 		...
 		STRUCT (struct user_lsmod, user_modules, FIELD(p, module_names) FIELD(p, module_) FIELD(p, next) FIELD(u, lsmod_len));
        	...
        	printf("oopsie");
       		...
       		STRUCT (struct user_fpu_state, user_fpstate, FIELD(x, control_word) 
				...
				FIELD(x, state_word) FIELD(x, last_instr_opcode) FIELD(x, instr_pointer) FIELD(x, data_pointer) 
					...
					FIELD(x, ip_offset) FIELD(x, ip_selector) FIELD(x, operand_offset) FIELD(x, operand_selector) FIELD(p, fpu_state_));
